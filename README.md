...
- Easy Troubling and Management, because all devices link to a central point, if for example port 4 on the bub isn’t connecting you know what device or cable is at fault. 
...
### Disadvantages: 
- If the centralised device(hub or switch) fails/ breaks, the connected devices cant receive or send data. 
- Larger Networks i.e. organisation(s) that are inherently Scaling, often need more maintenance, which can and does effect the CIA Triad(Confidentiality, Integrity &, Availability)especially Availability of systems and networks for users & employees.
